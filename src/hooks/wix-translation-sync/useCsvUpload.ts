...
import { parseCsvText } from "@/lib/services/wix-translation-sync/csvParser";
import { MAX_FILE_SIZE_BYTES } from "@/constants/wix-translation-sync/translationTool";
...
export function useCsvUpload<T = any>(
  fileLabel: string
): UseCsvUploadResult<T> {
    ...
    async (uploadedFile: File) => {
      ...
      try {
        ...
        if (parseResult.errors.length > 0) {
          // Log parsing warnings but do not fail unless critical
